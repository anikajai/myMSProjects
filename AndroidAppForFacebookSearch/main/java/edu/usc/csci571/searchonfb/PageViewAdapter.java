...
public class PageViewAdapter extends ArrayAdapter<List> {
    private final Activity context;
    private final List<Info> listInfo;

    public PageViewAdapter(Activity context, List listInfo) {
        super(context, R.layout.fragment_page_list, listInfo);
    ...
    public View getView(final int position, View view, ViewGroup parent) {
        ...
        View rowView = inflater.inflate(R.layout.fragment_page_list, null, true);
        ImageView favImage = (ImageView) rowView.findViewById(R.id.fav_page_imageView);
        ...
        TextView title = (TextView) rowView.findViewById(R.id.page_name);
        ImageView imageView = (ImageView) rowView.findViewById(R.id.iconPage);
        title.setText(listInfo.get(position).name);
        Picasso.with(this.getContext()).load(listInfo.get(position).url).into(imageView);
        ImageView detailsImageView = (ImageView) rowView.findViewById(R.id.details_page_imageView);
        detailsImageView.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                MainActivity.favFragment.selectedTabId = 1;
