...
public class GroupViewAdapter extends ArrayAdapter<List> {

    private final Activity context;
    private final List<Info> listInfo;

    public GroupViewAdapter(Activity context, List listInfo) {
        super(context, R.layout.fragment_group_list, listInfo);
    ...
    public View getView(final int position, View view, ViewGroup parent) {
        ...
        View rowView = inflater.inflate(R.layout.fragment_group_list, null, true);
        final String id = listInfo.get(position).id;
        final boolean favoriteYes = isFavorite(id);
        ImageView favImage = (ImageView) rowView.findViewById(R.id.fav_group_imageView);
        if (favoriteYes) {
            favImage.setImageResource(R.mipmap.favorites_on);
        } else {
            favImage.setImageResource(R.drawable.favorites_off);
        }
        TextView title = (TextView) rowView.findViewById(R.id.group_name);
        ImageView imageView = (ImageView) rowView.findViewById(R.id.iconGroup);
        title.setText(listInfo.get(position).name);
        Picasso.with(this.getContext()).load(listInfo.get(position).url).into(imageView);
        ImageView detailsImageView = (ImageView) rowView.findViewById(R.id.details_group_imageView);
        detailsImageView.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                MainActivity.favFragment.selectedTabId = 4;
