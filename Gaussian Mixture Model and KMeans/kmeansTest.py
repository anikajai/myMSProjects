...
def transform_image(image, code_vectors):
    ...
    dupli_means = np.zeros((image.shape[0], image.shape[1], code_vectors.shape[0], code_vectors.shape[1]), dtype=float)
    dupli_x = np.zeros((image.shape[0],image.shape[1], 1, code_vectors.shape[1]), dtype=float)

    for i in range(dupli_means.shape[0]):
        for j in range(dupli_means.shape[1]):
            dupli_means[i][j] = code_vectors
            dupli_x[i][j] = image[i][j]
    distances = np.square(np.linalg.norm(dupli_x - dupli_means, axis=3))
    codes = np.argmin(distances, axis=2)
    image_new = np.array(image)
    for i in range(codes.shape[0]):
        for j in range(codes.shape[1]):
            image_new[i][j] = code_vectors[codes[i][j]]
    return image_new    
    '''
    for i, row in enumerate(image):
        for j, pixel in enumerate(row):
            _min = float("inf")
            minIndex = -1
            for mu, code in enumerate(code_vectors):
                dist = np.linalg.norm(pixel-code)
                if (dist < _min):
                    _min = dist
                    minIndex = mu
            image[i][j] = code_vectors[minIndex]
    return image
    '''
    # TODO
