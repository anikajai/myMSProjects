...
def objective_function(X, y, w, lamb):
    ...
    part1 = np.matmul(w.T,w) * lamb/2
    temp = np.matmul(X, w)
    part2 = 1 -y*temp
    obj_value = part1 + np.sum(np.maximum(0,part2))/len(X)
...
def pegasos_train(Xtrain, ytrain, w, lamb, k, max_iterations):
    ...
    for iter in range(1, max_iterations + 1):
        ...
        neta = 1/(lamb*iter)
        part1 = (1-neta*lamb)*w
        part2 = (neta/k) * np.matmul(A_t_plus_x.T,A_t_plus_y)
        wt_half =part1 + part2
        w = np.minimum(1, 1/np.sqrt(lamb)/np.linalg.norm(wt_half))*wt_half
        
        train_obj.append(objective_function(X,y,w, lamb))
...
def pegasos_test(Xtest, ytest, w, t = 0.):
    ...
    num=0
    Y = np.array(ytest)
    for ind, cls in enumerate(Y):
        if cls == pred[ind]:
            num=num+1
    #misclassified = np.sum(Y==pred)
    #print (misclassified)
    #p
    #print (acc)
    test_acc=num/len(Y)             
