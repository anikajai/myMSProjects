...
class Perceptron:
    ...
    def train(self, features: List[List[float]], labels: List[int]) -> bool:
        ...
        for i in range(0,self.max_iteration):
            for index, feature in enumerate(feature_array):
                prediction = np.matmul(weights,np.transpose(feature))
                if ((prediction <= 0 and labels[index] == 1) or (prediction > 0 and labels[index] == -1)):
                    #weight update call
                    den_norm = np.linalg.norm(feature)
                    weights = weights + (labels[index]*feature)/den_norm
        self.w = weights.tolist()
        #print (self.w)
        for index, feature in enumerate(feature_array):
            prediction = np.matmul(weights,np.transpose(feature))
            if ((prediction <= 0 and labels[index] == 1) or (prediction > 0 and labels[index] == -1)):
    ...
    def predict(self, features: List[List[float]]) -> List[int]:
        ...
        output_labels = []
        #for index, feature in enumerate(feature_array):
        prediction = np.matmul(weights,np.transpose(feature_array))
